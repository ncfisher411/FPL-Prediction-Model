# Get league matches, clean sheets for 2020/21 season
...
#write.csv(df, file="match lineups/lineups2021.csv", row.names = F)
df <- read.csv("match lineups/lineups2021.csv")
gc()

#df2 <- fb_match_report(ids)
#write.csv(df2, file="game_results/results12021.csv")
df2 <- read.csv("game_results/results12021.csv")
gc()
...
write.csv(df4, file = "outputs/time_and_cs_2021.csv", row.names = F)
write.csv(df5, file="outputs/own_goals_2021.csv", row.names = F)
