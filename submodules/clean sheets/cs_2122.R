# Get league matches, clean sheets for 2021/22 season
...
#ids <- unique(df$MatchURL)

#df <- fb_match_lineups(ids)
#write.csv(df, file="match lineups/lineups2022.csv", row.names = F)
df <- read.csv("match lineups/lineups2022.csv")
gc()

#df2 <- fb_match_report(ids)
#write.csv(df2, file="game_results/results2022.csv")
df2 <- read.csv("game_results/results2022.csv")
gc()
...
write.csv(df4, file = "outputs/time_and_cs_2122.csv", row.names = F)
write.csv(df5, file="outputs/own_goals_2122.csv", row.names = F)
