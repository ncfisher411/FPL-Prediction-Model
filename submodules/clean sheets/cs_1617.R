# Get league matches, clean sheets for 2016/17 season
...
#write.csv(df, file="match lineups/lineups1617.csv")
df <- read.csv("match lineups/lineups1617.csv")
gc()

#df2 <- fb_match_report(match_url = ids)
#write.csv(df2, "game_results/results1617.csv", row.names = F)
df2 <- read.csv("game_results/results1617.csv")
gc()
...
write.csv(df4, file = "outputs/time_and_cs_1617.csv", row.names = F)
write.csv(df5, file="outputs/own_goals_1617.csv", row.names = F)
