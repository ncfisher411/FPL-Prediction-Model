# Get league matches, clean sheets for 2022/23 season
...
#write.csv(df, file="match lineups/lineups2023.csv", row.names = F)
df <- read.csv("match lineups/lineups2023.csv")
gc()

#df2 <- fb_match_report(ids)
#write.csv(df2, file="game_results/results2023.csv")
df2 <- read.csv("game_results/results2023.csv")
gc()
...
write.csv(df4, file = "outputs/time_and_cs_2223.csv", row.names = F)
write.csv(df5, file="outputs/own_goals_2223.csv", row.names = F)
