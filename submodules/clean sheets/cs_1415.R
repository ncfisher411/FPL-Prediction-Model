# Get league matches, clean sheets for 2014/15 season
...
#write.csv(df, file="match lineups/lineups1415.csv", row.names = F)
df <- read.csv("match lineups/lineups1415.csv")
gc()

#df2 <- fb_match_report(match_url = ids)
#write.csv(df2, file="game_results/results1415.csv", row.names = F)
df2 <- read.csv("game_results/results1415.csv")
gc()
...
write.csv(df4, file = "outputs/time_and_cs_1415.csv", row.names = F)
write.csv(df5, file="outputs/own_goals_1415.csv", row.names = F)