# Get league matches, clean sheets for 2018/19 season
...
#write.csv(df, file="match lineups/lineups1819.csv")
df <- read.csv("match lineups/lineups1819.csv")
gc()

#df2 <- fb_match_report(ids)
#write.csv(df2, file="game_results/results1819.csv", row.names = F)
df2 <- read.csv("game_results/results1819.csv")
gc()
...
write.csv(df4, file = "outputs/time_and_cs_1819.csv", row.names = F)
write.csv(df5, file="outputs/own_goals_1819.csv", row.names = F)
